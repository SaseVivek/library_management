...
CREATE TABLE member_issue_summary AS
SELECT
    m.member_id,
    m.member_name,
...
-- question 17 --Total Books Issued per Member
...
---question18-- Books Issued per Branch

CREATE TABLE branch_issue_summary AS
SELECT
    b.branch_id,
    b.branch_address,
    COUNT(i.issued_id) AS total_books_issued
...
--question19--Monthly Book Issue Summary


CREATE TABLE monthly_issue_summary AS
SELECT
    DATE_FORMAT(i.issued_date, '%Y-%m') AS issue_month,
    COUNT(i.issued_id) AS total_books_issued
FROM
    issued_status i
GROUP BY
...
--question20-- Top 10 Most Issued Books

CREATE TABLE top_10_books AS
SELECT
    b.isbn,
    b.book_title,
